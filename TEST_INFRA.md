...
> | `replay.spec.ts` | Analysis view (#85): empty state; a recorded round replays trick by trick from the stored trick log with all four hands face-up, banked points, trick-winner marking, jump-to-end result and rewind | not in the original catalogue (the analysis view wasn't enumerated as a feature) |
