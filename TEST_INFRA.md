...
> | `avatar-sync.spec.ts` | Profile pictures (#14) across the real WebRTC channel both ways (host's custom data URL → guest, guest's preset → host via `CONNECTION_ACK`); a custom picture is sent once as `AVATAR_UPDATE` and never repeated in a state snapshot; an oversized avatar from a peer is rejected and falls back to the default; an avatar smuggled into a state snapshot (bypassing `AVATAR_UPDATE`) is rejected by the guest | not in the original catalogue (profile pictures weren't enumerated as a feature) |
...
> | `replay.spec.ts` | Analysis view (#85): empty state; a recorded round replays trick by trick from the stored trick log with all four hands face-up, banked points, trick-winner marking, jump-to-end result and rewind; playback rolling across round borders through a whole list | not in the original catalogue (the analysis view wasn't enumerated as a feature) |
