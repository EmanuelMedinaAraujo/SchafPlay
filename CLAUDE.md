...
- **`protocol.ts`** defines the wire shape: `createMessage(type, payload)` stamps a timestamp. Types are host→guest `GAME_STATE_UPDATE`, guest→host `PLAYER_ACTION`, guest→host-on-connect `CONNECTION_ACK` (guest name + avatar), host→guest `AVATAR_UPDATE` (the seat→profile-picture map, #14), plus transport-level `PING`/`PONG`.
...
- **Profile pictures on the wire** (`session/avatarSync.ts`, #14): `Player.avatar` is public (untouched by `redactStateFor`) and rides in the state, but a *custom* avatar is a ~25 kB data URL and the host broadcasts a full snapshot dozens of times per round. So the host strips `data:` avatars out of the broadcast state and sends the seat→avatar map once per connection (`AVATAR_UPDATE`); the guest merges it back into every state it receives. `preset:` keys are a dozen bytes and stay in the state, so a preset always renders even before the map lands. This module is also the **validation boundary** for avatars off the wire — `sanitizeAvatar` caps the length (`MAX_AVATAR_LENGTH`, defined in `lib/avatars.ts` so the uploader shares the number) and accepts only a short well-formed `preset:` key or a raster `data:image/*` URL, so a buggy or hostile peer cannot pin megabytes into the authoritative state. The engine stores whatever the session hands it; the session sanitizes, exactly as it already overwrites the action `playerId`. There are **three** inbound paths and all three are sanitized: `CONNECTION_ACK.avatar` on the host, and — on the guest — both `AVATAR_UPDATE` (`sanitizeAvatarMap`, which also drops any key that isn't one of the four seats) *and* the avatars carried inside `GAME_STATE_UPDATE` (`sanitizeStateAvatars`). That last one is easy to overlook and not optional: sanitizing only the out-of-band map is bypassable by construction, since a hostile host can skip `AVATAR_UPDATE` and put the payload straight into `players[i].avatar`. An avatar is the one peer-supplied string that reaches an `<img src>`, so keep it out of CSS `url()` and `href` sinks.
...
- **Device settings** (`src/lib/settings.ts`): every persisted local preference — `language`, `playerName`, `avatar` (#14), `totalRounds`, `disableLaufende`, `lastMode` (#44) — lives in one `Settings` shape behind a `SettingsStore` seam (the same swap-the-backend pattern as `GameHistoryStore`, but synchronous so the right value is on screen at first paint). `App.tsx` holds the single `useSettings()` and passes values + `updateSetting(key, value)` setters down. Add a new preference by extending `Settings` + `DEFAULT_SETTINGS` + the `CODECS` map (which owns the tolerant parse/serialize and the historical `schafplay.*` `localStorage` keys) — never hand-roll a `localStorage` read/write in a component again. The default `LocalStorageSettingsStore` degrades silently when storage is unavailable, same contract as the persistence layer.

## Testing

A Playwright E2E suite exists (`tests/e2e/*.spec.ts`, 39 tests, resolves issue #5) and **must be kept green** — it runs in CI (`.github/workflows/e2e.yml`) on every pull request and on push to `main`. It covers WebRTC pairing/reconnect, bidding legality, card-play rule enforcement (including the Sauspiel called-Ace restrictions and Davonlaufen), scored full rounds, a full 4-round list, partner-badge redaction over the wire, settings persistence, and stats recording. `TEST_INFRA.md` maps which documented test cases each spec file covers.
