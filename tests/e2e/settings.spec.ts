...
test.describe("settings", () => {
  ...
  test("last-used game mode is preselected after reload (#44)", async ({ page }) => {
    ...
    // Default mode is solo (#94) — no invite is minted before the player asks.
    ...
    await page.reload();
    await expect(page.getByRole("tab", { name: de.joinGame })).toHaveAttribute("aria-selected", "true");
    await expect(page.getByRole("tab", { name: de.soloGame })).toHaveAttribute("aria-selected", "false");
